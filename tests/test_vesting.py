...
def test_wrong_create_params(vesting, co, accounts, chain, minter):
    ...
    with brownie.reverts('Vesting: wrong vehicule parametres'):
        vesting.createVehicule(recipient, 10, 10, start + 200, start + 100, {'from':minter})
    with brownie.reverts('Vesting: start cannot be 0'):
        vesting.createVehicule(recipient, 10, 10, 0, start + 100, {'from':minter})
...
def test_add_vesting(vesting, co, accounts, chain, minter):
    ...
    with brownie.reverts('Ownable: caller is not the owner'):
        vesting.createVehicule(recipient, 10, 10, start, start + 100, {'from':accounts[1]})

    vesting.createVehicule(recipient, 10, 10, start, start + 100, {'from':minter})
    assert vesting.vehiculeCount(recipient) == 1
    assert vesting.vehicules(recipient,0) == (start, start + 100, 10, 10, 0, 0)
...
def test_claim_upfront(vesting, co, accounts, chain, minter):
    ...
    vesting.createVehicule(recipient, 10, upfront_claimable, start, start + 100, {'from':minter})
...
def test_start_in_future(vesting, co, accounts, chain, minter):
    ...
    recipient = accounts[9]
    assert vesting.vehiculeCount(recipient) == 0
    vesting.createVehicule(recipient, '100 ether', 0, start + 5000, start + 15000, {'from':minter})
    with brownie.reverts('Vesting: vehicule does not exist'):
        vesting.claim(0, {'from':accounts[2]})
    with brownie.reverts('Vesting: vehicule does not exist'):
        vesting.claim(0, {'from':minter})
...
def test_fully_unlocked(vesting, co, accounts, chain, minter):
    ...
    recipient = accounts[9]
    assert vesting.vehiculeCount(recipient) == 0
    vesting.createVehicule(recipient, claimable, 0, start , start + 1, {'from':minter})
    chain.sleep(10)
...
def test_claim_some(Vesting, TestToken, accounts, chain, delta):
    ...
    assert vesting.vehiculeCount(recipient) == 0
    pre_bal = co.balanceOf(recipient)
    vesting.createVehicule(recipient, claimable, 0, start , start + maxDelta, {'from':minter})
    chain.sleep(delta)
...
def test_claim_at_random_until_empty(vesting, co, accounts, chain, minter):
    ...
    recipient = accounts[9]
    assert vesting.vehiculeCount(recipient) == 0
    vesting.createVehicule(recipient, claimable, 0, start , start + duration, {'from':minter})
    elapsed = 0
    total_claimed = 0
...
def test_end_vehicule_before_start(vesting, co, accounts, chain, minter):
    ...
    assert vesting.vehicules(recipient,0) == (start + 5000, start + 15000, 0, 0, 0, 0)
...
def test_end_vehicule_after_start(vesting, co, accounts, chain, minter):
    ...
    recipient = accounts[9]
    assert vesting.vehiculeCount(recipient) == 0
    vesting.createVehicule(recipient, '100 ether', 0, start, start + 15000, {'from':minter})
    chain.sleep(7500)
    chain.mine()
    vesting.endVehicule(recipient, 0, {'from':minter})
    (s,e,upfront,amount,_,_) =  vesting.vehicules(recipient,0)
    assert e >= start + 7500
